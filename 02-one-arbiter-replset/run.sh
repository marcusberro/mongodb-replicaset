...
function startReplication() {
  ...
  while ! mongo --port 27020 --eval "var rs_cluster=$(cat ${scriptPath}/replication-cluster.json);" $scriptPath/start-replication.js; do 
    ((COUNT++));
    sleep 1; 
    echo " ";
    echo " ### Waiting for Mongo instance 27020 startup... ${COUNT}s"; 
    (($COUNT > $TIMEOUT)) && 
      echo " ### Cannot start replication: Timeout while trying to connect Mongo instance 27020" &&  
      exit 1;
  done
}
